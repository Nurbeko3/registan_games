...
export function CreateRoomModal({ onCreate, onClose }: { onCreate: (s: RoomSettings) => void; onClose: () => void }) {
  ...
  const create = () =>
    onCreate({ ...DEFAULT_SETTINGS, mapId, modeId, perTeam, difficulty, botFill, targetScore: getMode(modeId).targetScore });
  ...
  return (
    <Overlay onClose={onClose}>
      ...
      <Row label="Team size">
        <Pills items={TEAM_SIZES.map((t) => ({ id: String(t.perTeam), label: t.label }))} value={String(perTeam)} onPick={(id) => setPerTeam(Number(id))} />
      </Row>
      ...
      <Row label="Fill empty slots with bots">
        <button
